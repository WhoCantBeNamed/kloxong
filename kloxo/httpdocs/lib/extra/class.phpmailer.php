...
class PHPMailer
{
    ...
    protected function addAnAddress($kind, $address, $name = '')
    {
        ...
        if ($kind != 'Reply-To') {
            if (!\array_key_exists(\strtolower($address), $this->all_recipients)) {
                ...
                $this->all_recipients[\strtolower($address)] = \true;
                return \true;
            }
        } else {
            ...
            foreach ($list as $address) {
                ...
                if (\strpos($address, '<') === \false) {
                    ...
                    if ($this->validateAddress($email)) {
                        $addresses[] = array('name' => \trim(\str_replace(array('"', "'"), '', $name)), 'address' => $email);
                    }
                }
            }
        }
    ...
    public function preSend()
    {
        try {
            ...
            if ((is_countable($this->to) ? \count($this->to) : 0) + (is_countable($this->cc) ? \count($this->cc) : 0) + (is_countable($this->bcc) ? \count($this->bcc) : 0) < 1) {
            ...
            if ($this->Mailer == 'mail') {
                if ((is_countable($this->to) ? \count($this->to) : 0) > 0) {
    ...
    protected function mailSend($header, $body)
    {
        ...
        $result = \false;
        if ($this->SingleTo and (is_countable($toArr) ? \count($toArr) : 0) > 1) {
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if ((is_countable($this->all_recipients) ? \count($this->all_recipients) : 0) > (is_countable($bad_rcpt) ? \count($bad_rcpt) : 0) and !$this->smtp->data($header . $body)) {
        ...
        if ((is_countable($bad_rcpt) ? \count($bad_rcpt) : 0) > 0) {
            $errstr = '';
    ...
    public function smtpConnect($options = \null)
    {
        ...
        foreach ($hosts as $hostentry) {
            $hostinfo = array();
            if (!\preg_match('/^((ssl|tls):\/\/)*([a-zA-Z0-9\.-]*):?([0-9]*)$/', \trim($hostentry), $hostinfo)) {
    ...
    public function createHeader()
    {
        ...
        if ($this->SingleTo) {
            ...
            } elseif ((is_countable($this->cc) ? \count($this->cc) : 0) == 0) {
                $result .= $this->headerLine('To', 'undisclosed-recipients:;');
            }
        }
        $result .= $this->addrAppend('From', array(array(\trim($this->From), $this->FromName)));
        // sendmail and mail() extract Cc from the header before sending
        if ((is_countable($this->cc) ? \count($this->cc) : 0) > 0) {
            $result .= $this->addrAppend('Cc', $this->cc);
        }
        // sendmail and mail() extract Bcc from the header before sending
        if (($this->Mailer == 'sendmail' or $this->Mailer == 'qmail' or $this->Mailer == 'mail') and (is_countable($this->bcc) ? \count($this->bcc) : 0) > 0) {
            $result .= $this->addrAppend('Bcc', $this->bcc);
        }
        if ((is_countable($this->ReplyTo) ? \count($this->ReplyTo) : 0) > 0) {
        ...
        foreach ($this->CustomHeader as $header) {
            $result .= $this->headerLine(\trim($header[0]), $this->encodeHeader(\trim($header[1])));
        }
        ...
        if ($this->isError()) {
            ...
            try {
                ...
                if (empty($this->sign_extracerts_file)) {
                    ...
                    $sign = @\openssl_pkcs7_sign($file, $signed, 'file://' . \realpath($this->sign_cert_file), array('file://' . \realpath($this->sign_key_file), $this->sign_key_pass), \null, \PKCS7_DETACHED, $this->sign_extracerts_file);
                }
                if ($sign) {
                    @\unlink($file);
    ...
    public function encodeHeader($str, $position = 'text')
    {
        ...
        if ($matchcount > \strlen($str) / 3) {
            ...
            $encoded = $this->wrapText($encoded, $maxlen, \true);
    ...
    {
        if ((is_countable($this->language) ? \count($this->language) : 0) < 1) {
            $this->setLanguage('en');
    ...
    public function html2text($html, $advanced = \false)
    {
        ...
        return \html_entity_decode(\trim(\strip_tags(\preg_replace('/<(head|title|style|script)[^>]*>.*?<\/\1>/si', '', $html))), \ENT_QUOTES, $this->CharSet);
    }
    /**
    ...
    public function DKIM_Sign($signHeader)
    {
        ...
        if (\version_compare(\PHP_VERSION, '5.3.0') >= 0 and \in_array('sha256WithRSAEncryption', \openssl_get_md_methods(\true))) {
