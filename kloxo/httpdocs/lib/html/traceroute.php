...
class traceroute extends \lxclass
{
    ...
    static function initThisList($parent, $class)
    {
        ...
        $_result = \rl_exec_get(\null, "localhost", array("traceroute", "exec_traceroute"), array($cmd));
        if (!\is_array($_result)) {
            throw new \lxException($login->getThrow("traceroute_failed"), '', $host);
        }
        if ((is_countable($_result) ? \count($_result) : 0) == 0) {
            throw new \lxException($login->getThrow("traceroute_failed"), '', $host);
        }
    ...
    {
        $this->_argRelation = array("linux" => array("numeric" => "-n", "ttl" => "-m", "deadline" => "-w"), "windows" => array("numeric" => "-d", "ttl" => "-h", "deadline" => "-w"));
    }
    function _parseResultlinux()
    {
        $raw_data_len = is_countable($this->_raw_data) ? \count($this->_raw_data) : 0;
        $dataRow = 0;
        while (empty($this->_raw_data[$dataRow]) && $dataRow < $raw_data_len) {
            $dataRow++;
        }
        $tempparts = \explode(' ', $this->_raw_data[$dataRow]);
        $this->_target_ip = \trim($tempparts[3], ' (),');
        ...
        while ($dataRow < $raw_data_len && !empty($this->_raw_data[$dataRow])) {
            ...
            if ((is_countable($parts) ? \count($parts) : 0) > 0) {
                ...
                if ((is_countable($machineparts) ? \count($machineparts) : 0) > 1) {
                    $hop['machine'] = $machineparts[0];
                    $hop['ip'] = \trim($machineparts[1], ' ()');
                } else {
            ...
            $responsetimes = array();
            for ($timeidx = 0; $timeidx < (is_countable($parts) ? \count($parts) : 0); $timeidx++) {
    ...
    function _parseResultwindows()
    {
        ...
        $tempparts = \explode(' ', $this->_raw_data[$dataRow]);
        $searchIdx = 0;
        while ($searchIdx < (is_countable($tempparts) ? \count($tempparts) : 0) && \substr($tempparts[$searchIdx], 0, 1) != '[') {
            $searchIdx++;
        }
        $this->_target_ip = \trim($tempparts[$searchIdx], ' [],');
        while ($searchIdx < (is_countable($tempparts) ? \count($tempparts) : 0) && (int) $tempparts[$searchIdx] <= 0) {
            $searchIdx++;
        }
        if ((int) $tempparts[$searchIdx] > 0) {
            $this->_ttl = (int) $tempparts[$searchIdx];
            // TTL might be written in next line; e.g. on Windows 98
        ...
        while ($dataRow < $raw_data_len && !empty($this->_raw_data[$dataRow])) {
            ...
            for ($timeidx = 0; $timeidx < 3; $timeidx++) {
                $temppart = \trim(\str_replace(' ms', '', \substr($this->_raw_data[$dataRow], 3 + $timeidx * 9, 9)));
                if ($temppart == '*') {
                    $responsetimes[] = -1;
                    // unreachable
                } else {
            ...
            // if we can find a next hop it's name/ip will be here
            if ((is_countable($machineparts) ? \count($machineparts) : 0) == 1) {
                $hop['ip'] = \trim($machineparts[0], ' ()[]');
            } elseif ((is_countable($machineparts) ? \count($machineparts) : 0) == 2) {
                $hop['machine'] = $machineparts[0];
                $hop['ip'] = \trim($machineparts[1], ' ()[]');
            }
