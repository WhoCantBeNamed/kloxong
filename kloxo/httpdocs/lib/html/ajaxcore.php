...
function __ajax_desc_tree()
{
	...
	if ($object->hasFileResource()) {
		...
		$ret[] = array('text'=> "File", 'icon' => "{$icondir}/ffile_show.gif", 'hrefTarget' => 'mainframe', 
			'href' => $u, 'id'=> "{$cid}&{$v}");
	}
	...
	if ($ghtml->__resource_class) {
		...
		if (cse($c, "_l")) {
			...
			foreach($list as $o) {
				...
				$u = $ghtml->getFullUrl($u);
				$ret[] = array('text' => basename($o->nname), 'icon' => "$icondir/{$o->getClass()}_list.gif", 
					'hrefTarget' => 'mainframe', 'href' => $u, 'id'=> "{$cid}&{$o->getClName()}");
			}
			...
			$u = $ghtml->getFullUrl($u);
			$ret[] = array('text' => $o->getClass(), 'icon' => "{$icondir}/{$o->getClass()}_show.gif", 
				'hrefTarget' => 'mainframe', 'href' => $u, 'id'=> "{$cid}&{$o->getClass()}");
		}
	...
	if ($ghtml->__title_function) {
		...
		foreach($alist as $k => $v) {
			...
			if ($insidetitle) {
				...
				$ret[] = array('text' => $urlinfo['description']['desc'], 'icon' => $urlinfo['image'], 
					'hrefTarget' => 'mainframe', 'leaf' => true, 'href' => $url, 'id'=> "&end");
			}
		}
	...
	foreach($rclist as $c) {
		...
		$ret[] = array('text' => $desc, 'icon' => "$icondir/{$clname}_list.gif", 'hrefTarget' => 'mainframe', 
			'href' => $url, 'id'=> "{$cid}&__resource_$c");
	}
...
function __ajax_desc_list()
{
	...
	$object = $gbl->__c_object;
	$description = $ghtml->getActionDetails("/display.php?{$ghtml->get_get_from_current_post(null)}", null, 
		$buttonpath, $path, $post, $file, $name, $image, $__t_identity);
	$img = "<img src=$image>";
	do_list_class($object, $ghtml->frm_o_cname);
	$v = ob_get_clean();
	$v = "{$img}{$v}";
	$ret = array('ajax_dismiss' => true, 'ajax_form_name' => null, 'ajax_need_var' => $gbl->__ajax_need_var, 
		'allbutton' => $gbl->__ajax_allbutton, 'lx__form' => $v);
...
function __ajax_desc_addform()
{
	...
	$v = ob_get_clean();
	$v = "{$img}{$v}";
	$ret = array('ajax_form_name' => $gbl->__ajax_form_name, 'ajax_need_var' => $gbl->__ajax_need_var,  
		'allbutton' => $gbl->__ajax_allbutton, 'lx__form' => $v);
...
function __ajax_desc_update()
{
	...
	try {
		...
		$ret = array('returnvalue' => 'failure', 'refresh' => false, 
			'url' => "{$url}&frm_ev_list={$evlist}&frm_emessage={$e->getMessage()}&frm_m_emessage_data=$e->value");
