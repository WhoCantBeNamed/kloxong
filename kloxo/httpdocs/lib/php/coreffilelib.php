...
class coreFfile
{
    ...
    static function getRealpath($_path)
    {
        ...
        */
        ...
        for ($i = 0; $i < (is_countable($__parts) ? \count($__parts) : 0); $i++) {
            if (\strlen($__parts[$i]) === 0 || $__parts[$i] === ".") {
                continue;
            }
            if ($__parts[$i] === "..") {
                if ((is_countable($__realparts) ? \count($__realparts) : 0) > 0) {
    ...
    static function getLxStat($__username_o, $root, $path, $numlines = \null, $duflag = \null)
    {
        \dprint("{$path}\n");
        self::check_for_break($root, $path);
    ...
    {
        if ($stat['ttype'] === "zip" || $stat['ttype'] === 'tar' || $stat['ttype'] === 'tgz' || $stat['ttype'] === 'tbz2' || $stat['ttype'] === 'txz' || $stat['ttype'] === 'p7z' || $stat['ttype'] === 'rar') {
            ...
            if ($stat['ttype'] === 'gz' || $stat['ttype'] === 'bz2' || $stat['ttype'] === 'xz') {
    ...
    static function getFile($stat, $file, $lines = \null)
    {
