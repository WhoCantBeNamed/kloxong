...
class TLDExtract
{
     ...
     */
    public function __construct($fetch = \true, $cacheFile = '')
    {
        $this->fetch = $fetch;
        $this->cacheFile = !empty($cacheFile) ? $cacheFile : __DIR__ . \DIRECTORY_SEPARATOR . '.tld_set';
    }
    /**
    ...
    private function fetchTldList()
    {
        ...
        if (!empty($page) && \preg_match_all('@^(?P<tld>[.*!]*\w[\S]*)@um', $page, $matches)) {
            $tlds = \array_fill_keys($matches['tld'], \true);
        }
        return $tlds;
    }
    private function fetchPage($url)
    {
        if (\ini_get('allow_url_fopen')) {
            ...
            if (\is_callable('curl_exec')) {
                ...
                \curl_setopt_array($handle, array(\CURLOPT_RETURNTRANSFER => \true, \CURLOPT_HEADER => \false, \CURLOPT_FAILONERROR => \true));
    ...
    public function extract($host)
    {
        ...
        for ($i = 0; $i < \count($parts); $i++) {
            ...
            if (\array_key_exists($exceptionTld, $this->tlds)) {
                return array(\join('.', \array_slice($parts, 0, $i + 1)), \join('.', \array_slice($parts, $i + 1)));
            }
            $wildcardTld = '*.' . \join('.', \array_slice($parts, $i + 1));
            if (\array_key_exists($wildcardTld, $this->tlds) || \array_key_exists($maybeTld, $this->tlds)) {
                return array(\join('.', \array_slice($parts, 0, $i)), $maybeTld);
            }
        }
        return array($host, '');
    }
