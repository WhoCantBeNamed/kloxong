...
if (!\defined('HN_REWRITECLASS')) {
    ...
    class hn_urlRewrite
    {
        ...
        // PUBLIC
        public $separator = '--';
        public $registered_scripts = \FALSE;
        public $rewrite_protected_scripts = array();
        public $hostnames = array();
        public $rewrite_extern_hostnames = \FALSE;
        public $valid_php_fileextensions = array('php');
        public $noimgs = \FALSE;
        public $nomaps = \FALSE;
        public $nourls = \FALSE;
        public $encode = \TRUE;
        public $strong_encode = \FALSE;
        public $CodingKey = array('F', 'G', 'R', 'U', 'L', 'B', 'K', 'N', 'T', 'W', 'E');
        public $add_indexhtml = 'index.html';
        public $add_pictjpg = 'pict.jpg';
        // PRIVATE
        public $host = '';
        public $path;
        public $compare;
        ...
        function hn_urlRewrite($registered_scripts = \FALSE)
        {
            ...
            if ($this->compare && (is_countable($this->registered_scripts) ? \count($this->registered_scripts) : 0) < 1) {
            ...
            if ($this->separator === \FALSE || \trim($this->separator) == '') {
        ...
        function _parse_path()
        {
            ...
            foreach ($params as $v) {
                $_GET[\strip_tags($v[0])] = \strip_tags($v[1]);
            }
            // build querystring
            ...
            if ((is_countable($params) ? \count($params) : 0) > 1) {
                for ($i = 1; $i < (is_countable($params) ? \count($params) : 0); $i++) {
        ...
        function buffer_start($HeadersAgainstCaching = \FALSE)
        {
            ...
            if (!\headers_sent() && $HeadersAgainstCaching) {
                \header("expires: Sun, 06 Jan 2002 01:00:00 GMT");
        ...
        {
            if ($compressed && preg_match('#gzip#m', $_SERVER["HTTP_ACCEPT_ENCODING"])) {
                // Browser kann komprimierte Daten verarbeiten:
                \header("Content-Encoding: gzip");
                // Content-Encoding senden (damit der Browser was merkt)
                echo "\37‹\10\0\0\0\0\0";
                // hmm, gzip-start?
                $Size = \strlen($this->page);
                // Groesse bestimmen
                $Crc = \crc32($this->page);
                // Checksumme bestimmen
                $this->page = \gzcompress($this->page, 9);
                // Komprimieren
                $this->page = \substr($this->page, 0, \strlen($this->page) - 4);
                // letzte 4 Bytes abschneiden
                echo $this->page;
                // komprimiertes Zeugs ausgeben
        ...
        function _calculate_UpLinks($url)
        {
            ...
            $k = is_countable($pathsegments) ? \count($pathsegments) : 0;
            $newurl = '';
        ...
        function _strong_decoded($str)
        {
            ...
            list($decoded, $letter) = preg_split('#\+#m', $decoded);
            for ($i = 0; $i < (is_countable($alpha_array) ? \count($alpha_array) : 0); $i++) {
        ...
        function _ErrorHandler($id = '')
        {
            switch ($id) {
                ...
                case 'no_file':
                    \header("Location: /404");
                    // DIRTY FIX! - Don't know, what other makes sense here?
                    exit(1);
                case 'no_path':
                    $this->path = '/';
                    // ALSO A DIRTY FIX!
