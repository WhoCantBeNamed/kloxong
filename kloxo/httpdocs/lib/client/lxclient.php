...
abstract class Lxclient extends \Lxdb
{
    ...
    function getThrow($var)
    {
        global $gbl, $sgbl, $login, $ghtml;
        global $g_language_mes;
        if (isset($g_language_mes->__throw[$var])) {
            return $g_language_mes->__throw[$var];
        }
        return $var;
    }
    function getThrowUc($var)
    {
        return \ucfirst($this->getThrow($var));
    }
    ...
    function notifyObjects($type)
    {
        ...
        // This actually returns itself most of the times. It is confusing and should be removed.
        // The parent is actully the first the pervious one itself in most of the cases.
    ...
    function sendNotification($noto, $obj, $subject, $txt, $parent = \null)
    {
    ...
    function updateBoxPosOpen($param)
    {
        ...
        $title_open = \isOn($param['title_open']) ? \true : \false;
    ...
    function getSkinColor()
    {
        ...
        if (\lxfile_exists("__path_program_htmlbase/{$dir}/base_color")) {
            $col = \trim(\lfile_get_contents("__path_program_htmlbase/{$dir}/base_color"));
        } else {
            $col = "EFE8E0";
        }
        return "{$col}";
    ...
    function updateform($subaction, $param)
    {
        ...
        switch ($subaction) {
            ...
            case "limit":
                ...
                // This is patently wrong. In update, the object is inititialized properly and we are suppsed to
    ...
    function updateDisable_per($param)
    {
        if ($param['disable_notify'] === 'on') {
            \touch('../etc/flag/disablenotifyforquota.flg');
        } else {
    ...
    function lxclientpostAdd()
    {
        ...
        $specialdisplay = new \sp_specialplay(\null, \null, $tsc->nname);
        foreach ($tsc as $k => $v) {
            $specialdisplay->{$k} = $v;
        }
        $this->ddate = \time();
