...
class phpini__sync extends \Lxdriverclass
{
    ...
    function createIniFile()
    {
        ...
        $input = array();
        foreach ($list as &$l) {
            $input[$l] = $this->main->phpini_flag_b->{$l} ?? '';
        }
        $user = $input['user'] = $this->main->__var_web_user ?? 'apache';
        $extrabasedir = $input['extrabasedir'] = $this->main->__var_extrabasedir ?? '';
    ...
    {
        $pclass = $this->main->getParentClass();
        $user = $this->main->__var_web_user ?? 'apache';
        $stlist[] = "###Start Kloxo PHP config Area";
        $stlist[] = "###Start Lxdmin Area";
        $stlist[] = "###Start Kloxo Area";
        $stlist[] = "###Start Lxadmin PHP config Area";
        ...
        if ($pclass === 'web') {
            ...
            $nowarning = \true;
            \file_put_between_comments("{$user}:apache", $stlist, $endlist, $startstring, $endstring, $htaccess_target, $htaccess_parse, $nowarning);
