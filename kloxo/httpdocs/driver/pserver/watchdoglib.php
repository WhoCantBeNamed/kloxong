...
class watchdog extends \lxdb
{
    ...
    */
    ...
    static function addOneWatchdog($pserver, $service, $port, $command, $status = \null)
    {
        $v = new \watchdog(\null, $pserver, "{$service}___{$pserver}");
        $v->get();
        if ($v->dbaction !== 'add') {
