...
HEAD;
        ...
        $mailrootpath = $sgbl->__path_mail_data;
        list($uid, $gid) = \explode(":", $sysuser);
        $mailpath = "{$mailrootpath}/domains/{$dom}";
        ...
        // MR -- use lxuser_return instead lxshell_return because automaticly chown to user
        $r = \lxuser_return($sysuser, "/usr/bin/ezmlm-make", "-ftx", "-5", $this->main->adminemail, "{$dom_dir}/", "{$mailpath}/.qmail-{$listname}", $listname, $dom);
    ...
    function dbactionDelete()
    {
        ...
        // MR -- lxfile_rm not work; change to lxfile_rm_rec
        \lxfile_rm_rec("{$mailpath}/.qmail-{$listname}");
        \lxfile_rm_rec("{$mailpath}/.qmail-{$listname}-default");
        \lxfile_rm_rec("{$mailpath}/.qmail-{$listname}-owner");
        \lxfile_rm_rec("{$mailpath}/.qmail-{$listname}-return-default");
    }
    ...
    function set_template($lang)
    {
        ...
        if (!\is_link($base_template)) {
            // test if symlink exist, if not then make english version and symlink it
            \rename($base_template, "{$base_template}.en");
            \symlink("{$base_template}.en", $base_template);
        }
        if (!\file_exists("{$base_template}.{$lang}")) {
            //test if language file exists, if not make one untranslated
            \copy("{$base_template}.en", "{$base_template}.{$lang}");
        }
        \unlink($base_template);
        //remove old symlink
        \symlink("{$base_template}.{$lang}", $base_template);
        //create a new one to the present language
    }
    function configureThis()
    {
        ...
        $flags .= "ftx";
        // prefix, trailer and mime remove are always active
